...
// ── Flow progress (generic, config-driven) ────────────────────────────────
...
const DEFAULT_FLOW_META = {
  ...
  steps: [
    { key: "STEP1_KNOW_YOUR_CUSTOMER", label: "Know Your Customer", order: 1, countable: true, totalQuestions: 6 },
    { key: "STEP2_NARROW_DOWN_USE_CASES", label: "Narrow Down Use Cases", order: 2, countable: true, totalQuestions: 1 },
    { key: "STEP3_PERFORM_DISCOVERY", label: "Perform Discovery", order: 3, countable: true, totalQuestions: 3 },
    { key: "STEP4_BUILD_READOUT", label: "Build Readout", order: 4, countable: true, totalQuestions: 1 },
    { key: "STEP5_READOUT_SUMMARY_NEXT_STEPS", label: "Readout Summary and Next Steps", order: 5, countable: true, totalQuestions: 1 },
  ],
};
...
export function computeFlowProgress(state: CfsState): FlowProgress {
  ...
  const steps: StepProgress[] = meta.steps.map((metaStep, idx) => {
    ...
    if (metaStep.countable) {
      if (metaStep.key === "STEP1_KNOW_YOUR_CUSTOMER") {
        answeredQuestions = status === "completed" ? totalQuestions : (questionKeyMap[state.session_context.last_question_key ?? ""] ?? 0);
      } else if (metaStep.key === "STEP2_NARROW_DOWN_USE_CASES") {
        const step2Cond = state.session_context.last_question_key === "S3_USE_CASE_SELECT" && state.session_context.awaiting_user;
        answeredQuestions = status === "completed" ? totalQuestions : status === "upcoming" ? 0 : (step2Cond ? 0 : 1);
      } else if (metaStep.key === "STEP4_BUILD_READOUT") {
        answeredQuestions = status === "completed" ? totalQuestions : (state.readout_context?.status === "ready" ? 1 : 0);
      } else if (metaStep.key === "STEP5_READOUT_SUMMARY_NEXT_STEPS") {
        answeredQuestions = status === "completed" ? totalQuestions : (state.readout_context?.status === "ready" ? 1 : 0);
      } else if (metaStep.key === dynamicCountStepKey) {
        ...
        if (status === "upcoming") {
          ...
          status === "completed" ||
          state.session_context.step === "STEP4_BUILD_READOUT" ||
          state.session_context.step === "STEP5_READOUT_SUMMARY_NEXT_STEPS" ||
          (total > 0 && responseCount >= total)
        ) {
          answeredQuestions = total;
        } else if (state.session_context.step === dynamicCountStepKey) {
